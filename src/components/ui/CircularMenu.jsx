    ...
    <>
          ...
          <motion.div
            ...
            <div
              ...
              <div className="flex flex-col gap-1 items-center justify-center">
                <div
                  className={`w-[2px] h-3 bg-white/70 rounded-full transition-transform duration-300 origin-bottom ${isPartsOpen ? "-rotate-45" : "rotate-45"}`}
                />
                <div
                  className={`w-[2px] h-3 bg-white/70 rounded-full transition-transform duration-300 origin-top ${isPartsOpen ? "rotate-45" : "-rotate-45"}`}
                />
              </div>
            </div>
          </motion.div>
      ...
      <AnimatePresence>
        {!isNavOpen && (!isMobile || isPartsOpen) && (
          <motion.div
            initial={
              isMobile ? { opacity: 0, x: -50 } : { opacity: 0, scale: 0.95 }
            }
      ...
      <div
        ...
        <div
          className={`w-6 h-[1px] bg-white transition-all duration-300 origin-center ${isNavOpen ? "rotate-45 translate-y-[7px]" : "group-hover:w-8 group-hover:bg-white"}`}
        />
        <div
          className={`w-6 h-[1px] bg-white transition-all duration-300 ${isNavOpen ? "opacity-0" : "group-hover:w-5 group-hover:bg-white"}`}
        />
        <div
          className={`w-6 h-[1px] bg-white transition-all duration-300 origin-center ${isNavOpen ? "-rotate-45 -translate-y-[7px]" : "group-hover:w-8 group-hover:bg-white"}`}
        />
        ...
        <div
          className={`absolute right-full mr-2 whitespace-nowrap text-[10px] font-medium tracking-[0.3em] transition-all duration-300 ${isNavOpen ? "text-white/0 -translate-x-2" : "text-white/50 group-hover:text-white translate-x-0"}`}>
          MENÜ
        </div>
      </div>
    </>
